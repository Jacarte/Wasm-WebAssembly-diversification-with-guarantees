...
\author[fastly-address]{Nick Fitzgerald,}[orcid=0000-0002-0209-2805]\ead{toady@eecs.kth.se}
...
Yet, the burgeoning ecosystem of WebAssembly compilers and tools has created a need for robust software diversification techniques.
Traditional approaches, often tied to specific compilation pipelines, can be restrictive in their scope and application.

We introduce \tool, a compiler-agnostic WebAssembly diversification engine. It is engineered to fulfill key criteria: the rapid generation of semantically equivalent yet behaviorally diverse WebAssembly variants, universal compatibility with existing WebAssembly programs, and the capability to counter high-risk security threats. 
Utilizing an e-graph data structure, \tool pioneers its usage in the realm of software diversification.
Our assessments reveal that \tool can efficiently generate tens of thousands of unique WebAssembly variants in a matter of minutes.
Additionally, it can produce variants with distinct machine code instruction traces and memory profiles in milliseconds. 
Notably, \tool's generated variants are fortified against Spectre attacks. 
This research not only delivers a potent tool for WebAssembly diversification but also contributes valuable insights for the development of more secure and resilient WebAssembly applications.
\end{abstract}
...
\wasm has also been adopted outside of browsers, with world-leading execution platforms like Fastly using it as a foundational technology for their content delivery network \cite{fastly}.
...
To the best of our knowledge, this work is the first to use an e-graph  for software diversification. An e-graph offers one essential property for diversification:  every path through the e-graph represents a functionally equivalent variant of the input program.  
...
Our security security assessment of  \tool consists in evaluating the degree to which diversification can mitigate Spectre attacks. This assessment is made with \wasm programs that have been previously identified as vulnerable to Spectre attacks \cite{Swivel}.
...
\begin{itemize}
    ...
    \item Empirical evidence on the diversity of variants created by \tool, both in terms of static binaries and execution traces.
    \item Demonstration that \tool can protect \wasm binaries against timing side-channel attacks, specifically, Spectre.
...
\begin{figure*}[h!]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/workflow.v2.pdf}
...
\subsection{\tool in practice}

\tool  is designed to generate a variant of a given WebAssembly program. 
These Wasm variants are not only unique and preserved but also maintain high fidelity to the original program while exhibiting different behavior. 
Below, we detail how \tool ensures these characteristics in a practical setting. 
The pseudo-code illustrating this process can be found in Algorithm \autoref{toolinpractice}, with \tool playing a pivotal role in line 8.

The algorithm initiates by executing the original Wasm program and capturing its execution traces, as shown in line 5. 
Following this, an infinite loop commences to stack transformations, as indicated in lines 8 and 9. 
If a unique variant is successfully generated, a JIT component for the Wasm engine is employed to produce the corresponding machine code, as seen in line 11.
Subsequently, if the generated machine code differs from the original, meaning a preservation of the newly introduced code transformation, the variant Wasm is executed to gather low-level traces. 
If these traces are distinct from the original traces (captured in line 5), as verified in line 15, the loop terminates and returns the newly generated variant.

Note that the algorithm described is highly versatile. 
\tool can be adapted to achieve different objectives; for instance, it has been employed as an obfuscation tool by Cabrera et al. \cite{CABRERAARTEAGA2023103296}.
...
\begin{algorithm}
	...
	%	\Procedure{MyProcedure}{}
        ...
        \\
        \State $originalTrace \gets \textbf{engine.execute}(originalWasm)$
        ...
        \While{true}
             ...
             \If{ $variantWasm$ is unique}
                ...
                \If{$variantJIT$ is unique}
                  \\
                    \State $trace \gets \textbf{engine.execute}(variantJIT)$
...
Given an original \wasm program P, and an its execution trace $T_1$, the time until different trace is defined as the time between the diversification process starts and the when the variant $V$ is generated with execution trace $T_2$, and $T_1 \neq T_2$.
\\
...
In Algorithm \ref{toolinpractice}, we quantify the elapsed time between line 6 and line 16 to obtain the time it takes for \tool to generate a unique \Wasm variant producing different execution traces.
...
For all original program, \tool succeeds in  generating one variant with different traces comparing to the original program, either in machine code instructions or memory access, ie both cumlative distributions readch 100%
...
\begin{tcolorbox}[boxrule=1pt,arc=.3em,boxsep=-1.3mm]
  ...
  The quickest time for generating a variant with a unique machine code trace is 0.12 seconds, and for divergent memory traces, it's best time is 0.06 seconds. 
  On average, the median time required to produce a variant with distcint traces stands at 5.4 seconds for unique machine code traces and 16.2 seconds for different memory traces. 
